...
resourcename="systemresourcemonitor"  # lower case, only
resourcegroup="systemresourcemonitor_rg"
...
# git clone https://github.com/apilogicserver/systemresourcemonitor.git
...
az group create --name systemresourcemonitor_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group systemresourcemonitor_rg --sku S1 --is-linux
az container create --resource-group systemresourcemonitor_rg --name systemresourcemonitor --image apilogicserver/systemresourcemonitor:latest --dns-name-label systemresourcemonitor --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//systemresourcemonitor.westus.azurecontainer.io:5656'
