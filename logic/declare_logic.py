...
def declare_logic():
    ...
    # Derive user's total uploads as the sum of associated FileUploads
    Rule.count(derive=User.file_upload_count, as_count_of=FileUpload)

    # Ensure the CPU log usage does not exceed 100%.
    Rule.constraint(validate=CpuLog, as_condition=lambda row: row.usage_percent <= 100, error_msg="CPU usage {row.usage_percent} exceeds 100%")
